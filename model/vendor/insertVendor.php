<?php
	...
	if(isset($_POST['vendorDetailsStatus'])){
		...
		$state = htmlentities($_POST['vendorDetailsVendorState']);
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$sql = 'INSERT INTO vendor(fullName, email, mobile, address, address2, city, state, status) VALUES(:fullName, :email, :mobile, :address, :address2, :city, :state, :status)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'address' => $address, 'address2' => $address2, 'city' => $city,'state' => $state,'status' => $status]);
