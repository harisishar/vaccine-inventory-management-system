	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				$insertItemSql = 'INSERT INTO item(itemNumber, itemName, stock, unitPrice, status, description) VALUES(:itemNumber, :itemName, :stock, :unitPrice, :status, :description)';
				$insertItemStatement = $conn->prepare($insertItemSql);
				$insertItemStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'stock' => $quantity, 'unitPrice' => $unitPrice, 'status' => $status, 'description' => $description]);
